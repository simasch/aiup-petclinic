...
- Extends `SpringBrowserlessTest`, carries `@SpringBootTest`, `@Import(TestcontainersConfiguration.class)` and
  `@Transactional` for per-test rollback — subclasses add none of these.
...
- `navigateRaw("owners/999999")` navigates by location string without asserting the resulting view type — use it for
  not-found / error flows, where the typed `navigate(View.class, params)` would throw because the router rerouted.
...
- Class name `UC<NNN><UseCaseName>IT` for a use case, `TC<NNN><TestCaseName>IT` for a test case document under
  `docs/test_cases/`. Same package as the view under test.
...
- No `@Transactional` rollback across a real HTTP round-trip: data a test creates is cleaned up in `@AfterEach`
  (through the UI or targeted deletes, idempotent).
- `@UseCase` on every test method applies here too.
- Run with `./mvnw verify` (all) or `./mvnw verify -Dit.test=UC004FindOwnersByLastNameIT`;
