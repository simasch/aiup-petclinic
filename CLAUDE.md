...
The code is deliberately kept at **UC-001 only** (welcome page + `MainLayout`
shell) so the remaining use cases can be generated live from the specs. The
guidelines in `docs/guidelines/` describe the conventions every generated
feature must follow, even where no example exists in the code yet.
...
./mvnw verify -Dit.test=UC004FindOwnersByLastNameIT
./mvnw verify -Dit.test=UC004FindOwnersByLastNameIT -Dheadless=false   # watch the browser
```
...
- **Before implementing a use case, writing a view, adding a repository, or
  ...
  first. It covers package layout (`ui` / `domain` / `web`), jOOQ mapping
  patterns, Vaadin view conventions, the shell exception, form validation,
  error handling, and the `*Repository` stereotype rule.
