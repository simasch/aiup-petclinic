...
public class FindOwnersView extends VerticalLayout {
    ...
    public FindOwnersView(OwnerRepository ownerRepository) {
        ...
        Button findButton = new Button("Find Owner", click -> search());
        findButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        findButton.addClickShortcut(Key.ENTER);

        Button addOwnerButton = new Button("Add Owner", click ->
    ...
    private void search() {
        ...
        if (total == 1) {
            ...
            OwnerListing only = ownerRepository.findListingByLastNamePrefix(prefix, 0, 1).findFirst().orElseThrow();
            getUI().ifPresent(ui -> ui.navigate(
                    OwnerDetailsView.class, OwnerRouteParameters.forOwner(only.owner().id())));
            return;
        }
